...
Public Class rawstorereporform
    Private connectionString As String = "Server=180.1.1.6;Database=wmg-po;User Id=sa;Password=admin@WMG2024;"

    Private Sub rawstorereporform_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadComboBoxes()
    End Sub
    ...
    Private Sub btnSearchOther_Click(sender As Object, e As EventArgs) Handles btnSearchOther.Click
        ...
        Dim query As String = "SELECT " &
                          "    batch AS 'الرسالة', " &
                          "    lot, " &
                          "    SUM(weight) AS 'وزن صرف او مرتجع', " &
                          "    CONVERT(DATE, date) AS 'تاريخ الحركة', " &
                          "    worderid AS 'امر شغل', " &
                          "    username AS 'يوزر', " &
                          "    refno AS 'الاذن' " &
                          "FROM raw_to_inspect " &
                          "WHERE (1=1) " &
                          "    AND (@fromDate IS NULL OR CONVERT(DATE, date) >= @fromDate) " &
                          "    AND (@toDate IS NULL OR CONVERT(DATE, date) <= @toDate) " &
                          "    AND (@batchId IS NULL OR batch = @batchId) " &
                          "GROUP BY batch, lot, CONVERT(DATE, date), worderid, username, refno"
