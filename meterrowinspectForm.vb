...
Imports System.IO
Public Class meterrowinspectForm
    Private sqlServerConnectionString As String = "Server=180.1.1.6;Database=wmg-po;User Id=sa;Password=admin@WMG2024;"
    Private Sub meterrowinspectForm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        LoadWorderIDs()
    ...
    Private Sub cmbworder_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbworder.SelectedIndexChanged
        ...
        lblweightstore.Text = "وزن فعلى مخزن: N/A"

        ' SQL Query to retrieve details for the selected Work Order
        Dim query As String = "SELECT td.worderid, c.contractno, c.batch, td.qty_m, td.qty_kg,po.qtykgstore " & _
                              "FROM techdata td " & _
                              "LEFT JOIN Contracts c ON td.contract_id = c.ContractID " & _
                              "LEFT JOIN porow_toinspect po ON td.worderid = po.worderid " & _
                              "WHERE td.worderid = @worderid"

        ' SQL Query to check if the Work Order exists in porow_toinspect
        Dim checkQuery As String = "SELECT qty_m FROM porow_toinspect WHERE worderid = @worderid" ' Query to check qty_m for the selected Work Order
        ...
        Using conn As New SqlConnection(sqlServerConnectionString)
            Using cmd As New SqlCommand(query, conn)
                ...
                Try
                    ...
                    If reader.Read() Then
                        ...
                        lblweightstore.Text = "وزن فعلى مخزن: " & If(reader("qtykgstore") IsNot DBNull.Value, reader("qtykgstore").ToString(), "N/A")
                    End If
            ...
            Using checkCmd As New SqlCommand(checkQuery, conn)
                ...
                Try
                    Dim qtyM As Object = checkCmd.ExecuteScalar()
                    If qtyM IsNot Nothing AndAlso qtyM IsNot DBNull.Value Then
                        Dim qtyMValue As Double = Convert.ToDouble(qtyM) ' Convert to appropriate type
                        lblstatus.Text = "الكمية متر: " & qtyMValue.ToString()
                        lblstatus.ForeColor = Color.Green
                    Else
                        lblstatus.Text = "الكمية متر: لم يتم العثور على البيانات"
  ...
  Private Sub btnInsert_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btninsert.Click
        ' Validate that a Work Order is selected and txtm is not empty
        If String.IsNullOrEmpty(cmbworder.Text) Then
            MessageBox.Show("Please select a Work Order ID.")
            Return
        End If

        If String.IsNullOrEmpty(txtm.Text) OrElse Not IsNumeric(txtm.Text) Then
            MessageBox.Show("Please enter a valid quantity in meters.")
            Return
        End If
        ...
        If isAlreadyInserted Then
            ...
            query = "UPDATE porow_toinspect " & _
                    "SET qty_m = @qty_m, datetrans_inspect = @datetrans_inspect, username_inspect = @username_inspect " & _
                    "WHERE worderid = @worderid"
        Else
            ' Insert query if the record does not exist
            query = "INSERT INTO porow_toinspect (worderid, qty_m, datetrans_inspect, username_inspect) " & _
                    "VALUES (@worderid, @qty_m, @datetrans_inspect, @username_inspect)"
        End If
        ...
        Using conn As New SqlConnection(sqlServerConnectionString)
            Using cmd As New SqlCommand(query, conn)
                ...
                cmd.Parameters.AddWithValue("@worderid", cmbworder.Text)
                cmd.Parameters.AddWithValue("@qty_m", Convert.ToDecimal(txtm.Text))
                cmd.Parameters.AddWithValue("@datetrans_inspect", DateTime.Now) ' Use the current date and time
                cmd.Parameters.AddWithValue("@username_inspect", LoggedInUsername)
                ...
                Try
                    ...
                    If rowsAffected > 0 Then
                        lblstatus.Text = If(isAlreadyInserted, "تم التحديث بنجاح", "تم التسجيل بنجاح")
                        lblstatus.ForeColor = If(isAlreadyInserted, Color.Blue, Color.Green)
