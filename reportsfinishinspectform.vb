...
Public Class reportsfinishinspectform
    ...
    Private Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        ...
        Dim query As String = "SELECT fi.worder_id as 'أمر شغل', count(fi.roll) AS 'عدد الأتواب', SUM(fi.height) AS 'الطول', SUM(fi.weight) AS 'الوزن', c.contractno as 'رقم التعاقد', c.batch as 'رقم الرسالة', " &
                          "CASE " &
                          ...
                          "WHEN DATEPART(HOUR, fi.date) >= 23 OR DATEPART(HOUR, fi.date) < 8 THEN 'Shift 3' " &
                          "END AS 'الوردية', " &
                          "CASE " &
                          "WHEN DATEPART(HOUR, fi.date) >= 23 THEN CONVERT(date, DATEADD(day, -1, fi.date)) " &
        ...
        If dateFrom <= dateTo Then
            query &= " AND CASE WHEN DATEPART(HOUR, fi.date) >= 23 THEN CONVERT(date, DATEADD(day, -1, fi.date)) " &
                 "WHEN DATEPART(HOUR, fi.date) < 8 THEN CONVERT(date, DATEADD(day, -1, fi.date)) " &
        ...
        Select Case shift
            ...
            Case "Shift 3"
                query &= " AND (DATEPART(HOUR, fi.date) >= 23 OR DATEPART(HOUR, fi.date) < 8)"
            Case "Shift 1&2"
                query &= " AND DATEPART(HOUR, fi.date) >= 8 AND DATEPART(HOUR, fi.date) < 23"
            Case "Shift 2&3"
                query &= " AND (DATEPART(HOUR, fi.date) >= 16 OR DATEPART(HOUR, fi.date) < 8)"
            Case "Shift 1&3"
                query &= " AND (DATEPART(HOUR, fi.date) >= 8 AND DATEPART(HOUR, fi.date) < 16 OR DATEPART(HOUR, fi.date) >= 23 OR DATEPART(HOUR, fi.date) < 8)"
            Case "All Shifts"
                ' No additional condition needed for All Shifts
        End Select

        query &= " GROUP BY fi.worder_id, c.contractno, c.batch, " &
             "CASE WHEN DATEPART(HOUR, fi.date) >= 23 THEN CONVERT(date, DATEADD(day, -1, fi.date)) " &
             "WHEN DATEPART(HOUR, fi.date) < 8 THEN CONVERT(date, DATEADD(day, -1, fi.date)) " &
             "ELSE CONVERT(date, fi.date) END, " &
             "CASE WHEN DATEPART(HOUR, fi.date) >= 8 AND DATEPART(HOUR, fi.date) < 16 THEN 'Shift 1' " &
             "WHEN DATEPART(HOUR, fi.date) >= 16 AND DATEPART(HOUR, fi.date) < 23 THEN 'Shift 2' " &
             "WHEN DATEPART(HOUR, fi.date) >= 23 OR DATEPART(HOUR, fi.date) < 8 THEN 'Shift 3' END, us.user_ar"
